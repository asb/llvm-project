...
%externref_cell = type ptr addrspace(1)
%funcref_cell = type ptr addrspace(1)
...
define %externref @ir_local_externref(%externref %arg) {
; CHECK-LABEL: ir_local_externref:
; CHECK:         .functype ir_local_externref (externref) -> (externref)
...
; CHECK-NEXT:    local.get 1
; CHECK-NEXT:    # fallthrough-return
 %retval = alloca %externref, addrspace(1)
 store %externref %arg, %externref_cell %retval
 call void @inhibit_store_to_load_forwarding()
 %reloaded = load %externref, %externref_cell %retval
 ret %externref %reloaded
}

define %funcref @ir_local_funcref(%funcref %arg) {
; CHECK-LABEL: ir_local_funcref:
; CHECK:         .functype ir_local_funcref (funcref) -> (funcref)
...
; CHECK-NEXT:    local.get 1
; CHECK-NEXT:    # fallthrough-return
 %retval = alloca %funcref, addrspace(1)
 store %funcref %arg, %funcref_cell %retval
 call void @inhibit_store_to_load_forwarding()
 %reloaded = load %funcref, %funcref_cell %retval
 ret %funcref %reloaded
}
